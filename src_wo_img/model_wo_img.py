...
LastEditors: Aman
LastEditTime: 2022-04-01 15:00:15
'''
...
class MultiModalEncoder(nn.Module):
    def __init__(self, model_cfgs):
        ...
        assert self.topic_hidden_dim == self.text_hidden_dim, \
            "The hidden dim of topic and text must be equal."
        # for topic mlp
...
class MultiModalAttentionLayer(nn.Module):
    ...
    def forward(self, topic_output, text_output):
        ...
        for i in range(self.seq_len):
            ...
            atten = nn.Softmax(dim=-1)(torch.cat([topic_att, text_att], dim=1).permute(0,2,1)) # [batch_size, attention_dim, 3] => [batch_size, 1, 3]
            output = torch.bmm(atten, torch.cat([topic_output.transpose(0, 1), \
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        encoder_batch = {'topic': batch['topic_emb'].float(), \
                         'image': batch['r_embs'].transpose(0, 1).float(), \
                         'text': batch['r_embs'].transpose(0, 1).float()}
        batch_size = batch['r_embs'].size(0)
        device = batch['r_embs'].device
        seq_len = batch['r_embs'].size(1)
        ...
        mm_attention_output = self.mm_atten_layer(topic_output, text_inner_attention_output.transpose(0,1))
        ...
        return loss, text_kl_loss.mean(), outputs # [batch_size, seq_len+_max_seq_length, vocab_size]
