...
LastEditors: Aman
LastEditTime: 2021-11-25 11:28:52
'''
...
parser.add_argument("--lr", default=9.6e-03, type=float, help="Learning rate") # 9.6e-5, 9.7e-5 9.634346590126681e-05_5epochs
parser.add_argument("--dropout", default=0.1, type=float, help="Dropout rate")
parser.add_argument("--teacher_forcing_ratio", default=0.2, type=float, help="Teacher forcing ratio")
...
parser.add_argument("--log_path", default="../logs/misc.log", type=str, help="Log directory")
...
def main(trial=None):
    ...
    encoder = MyRNNsEncoder(model_cfgs, args.dropout)
    decoder = MyRNNsDecoder(model_cfgs, len(tokenizer.vocab), dropout=args.dropout)
...
def train(model, train_data, valid_data):
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in epoch_iterator:
            ...
            outputs = model.forward(batch, args.teacher_forcing_ratio) # [batch_size, seq_len, vocab_size]
            # pdb.set_trace()
...
def evaluate(model, valid_dataset, criterion):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(valid_dataset):
            batch = {k: v.to(device) for k, v in batch.items()}
            outputs = model.forward(batch) # [batch_size, seq_len, vocab_size]
