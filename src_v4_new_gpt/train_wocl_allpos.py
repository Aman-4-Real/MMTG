...
LastEditors: Aman
LastEditTime: 2022-01-25 03:00:09
'''
...
parser.add_argument("--save_path", default="./models/final_5ep_1e-5_bsz96_wocl_allpos-4", type=str, help="Save directory")
# parser.add_argument("--save_interval", default=1, type=int, help="Save interval")
parser.add_argument("--log_path", default="./logs/final_5ep_1e-5_bsz96_wocl_allpos-4.log", type=str, help="Log directory")
parser.add_argument("--tensorboard_log_dir", default="./logs/final_5ep_1e-5_bsz96_wocl_allpos-4", type=str, help="Tensorboard log directory")
...
class MyLoss(torch.nn.Module):
    ...
    def forward(self, outputs, targets, ratings):
        '''
        Args:
...
def train(model, train_data, valid_data):
    ...
    print('Total training steps:', training_steps * args.epochs)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in epoch_iterator:
            ...
            loss = criterion(outputs, targets, ratings)
            loss = loss.mean()
            ...
            if step > 0 and (step + 1) % int(len(train_dataset) * args.val_interval_ratio) == 0:
                val_loss, ppl_loss, ppl = evaluate(model, valid_dataset, criterion)
