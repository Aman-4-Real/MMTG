...
LastEditors: Aman
LastEditTime: 2022-01-22 15:03:13
'''
...
parser.add_argument("--save_path", default="./models/final_5ep_lr1e-5_bsz96_ln_newcl", type=str, help="Save directory")
# parser.add_argument("--save_interval", default=1, type=int, help="Save interval")
parser.add_argument("--log_path", default="./logs/final_5ep_lr1e-5_bsz96_ln_newcl.log", type=str, help="Log directory")
parser.add_argument("--tensorboard_log_dir", default="./logs/final_5ep_lr1e-5_bsz96_ln_newcl", type=str, help="Tensorboard log directory")
...
def train(model, train_data, valid_data):
    ...
    train_dataset = DataLoader(train_data, batch_size=batch_size, shuffle=False, num_workers=args.num_workers)
    valid_dataset = DataLoader(valid_data, batch_size=val_batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    # valid_datasets = [valid_dataset_1, valid_dataset_2]
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in epoch_iterator:
            ...
            if step > 0 and (step + 1) % int(len(train_dataset) * args.val_interval_ratio) == 0:
                val_loss, ppl_loss, ppl = evaluate(model, valid_dataset, stage, criterion)
