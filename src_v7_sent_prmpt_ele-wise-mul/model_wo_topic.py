'''
Author: Aman
Date: 2022-04-03 23:37:58
Contact: cq335955781@gmail.com
LastEditors: Aman
LastEditTime: 2022-04-04 13:18:58
'''
...
class GPT2_Decoder(nn.Module):
    ...
    def forward(self, concat_output, input_ids, \
        ...
        if is_train:
            ...
            input_embs = input_ids_wenlan

            # process type_ids: [1]*44 + [2]*44 + [3]*44 + [4]*44 + [1]*44
            type_ids = type_ids # [seq_len + seq_len * _sent_length * 2]
            
            # process attention mask
            attention_mask = attention_mask # [seq_len + seq_len * _sent_length * 2]
        ...
        # inference
            ...
            _type_ids = torch.tensor([]).unsqueeze(0).repeat(type_ids.size(0),1).to(type_ids.device)
            ...
            # add attention mask
            _attention_mask = torch.tensor([]).unsqueeze(0).repeat(attention_mask.size(0),1).to(attention_mask.device)
            for i in range(input_ids.size(1)):
