...
LastEditors: Aman
LastEditTime: 2022-01-25 11:16:25
'''
...
class GPT2_Decoder(nn.Module):
    ...
    def forward(self, concat_output, input_ids, \
        ...
        '''
        # process labels
        prompt_length = concat_output.size(1)
        ...
        input_embs = torch.cat([concat_output, input_ids_wenlan], dim=1) # [batch_size, topic_prompt_length + seq_len + seq_len * _sent_length * 2, 2048]
        ...
        type_ids = torch.cat([exp_prompt_type_ids, type_ids], dim=1).to(input_ids.device) # [seq_len + seq_len * _sent_length * 2]
        ...
        attention_mask = torch.cat([exp_att_mask, attention_mask], dim=1) # [seq_len + seq_len * _sent_length * 2]
