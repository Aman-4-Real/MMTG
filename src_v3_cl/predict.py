...
LastEditors: Aman
LastEditTime: 2021-12-29 20:27:47
'''
...
def calculate_sent_prob(model, start_input, label, seq_len, length, device):
    # import pdb; pdb.set_trace()
    inputs = start_input
    for k, v in inputs.items():
        if k == 'targets':
            ...
            # inputs[k] = torch.tensor(v, dtype=torch.float32, device=device)
            inputs[k] = v.clone().detach()
    generated = inputs['targets']
    probs = [[] for _ in range(seq_len)]
    max_probs = [[] for _ in range(seq_len)]
    with torch.no_grad():
        for t in range(length-1):
            ...
            for i in range(seq_len):
                ...
                next_tokens[i][0] = torch.tensor(label[i][t+1], dtype=torch.long, device=device)
            inputs['targets'] = torch.cat((generated[0], next_tokens), dim=-1).unsqueeze(0)
            # import pdb; pdb.set_trace()
        # generated = generated.tolist()[0]
...
def test(model, test_dataset, device):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(epoch_iterator):
            ...
            _, outputs = model.forward(batch) # [batch_size, seq_len, _max_sent_length*2, vocab_size]
...
def main():
    ...
    parser.add_argument("--batch_size", default=64, type=int, help="Test batch size")
    parser.add_argument("--seed", default=42, type=int, help="Random seed")
    parser.add_argument("--num_workers", default=4, type=int, help="Number of workers")
    parser.add_argument("--data_path", default="../datasets/data_rating/test_data_with_ratings_983.pkl", type=str, help="Data directory")
    parser.add_argument("--model_path", default="./models/test2/epoch_5.pth", type=str, help="Model path")
    ...
    seq_len = model_cfgs['seq_len'] # 5
    length = data_config.max_sent_length*2 # 40
    ...
    while True:
        ...
        label_tokens = [tokenizer.convert_ids_to_tokens(line) for line in label]
        ...
        for line in preds:
            while '[PAD]' in line:
                line.remove('[PAD]')
            print(''.join(''.join(line).split('[SEP]')[0])+'[SEP]')
            # print(line)
        print("-"*80)
        print("Label:")
        for line in label_tokens:
            ...
            print(''.join(line))
        print("-"*80)
        probs, maxprobs = calculate_sent_prob(model, start_input, label, seq_len, length, device)
        print("Probs:")
        probs = [sum(line) for line in probs]
        print(probs, np.mean(probs))
        print("Top-1 Probs:")
        maxprobs = [sum(line) for line in maxprobs]
        print(maxprobs, np.mean(maxprobs))
        print("="*100)
