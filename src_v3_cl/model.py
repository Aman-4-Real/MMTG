...
LastEditors: Aman
LastEditTime: 2021-12-29 20:28:54
'''
...
class SelfAttentionLayer(nn.Module):
    ...
    def reshape_for_scores(self, x):
        ...
        return x.permute(0, 2, 1, 3) # [bs, attention_heads, seq_len, attention_head_size]
...
class GPT2_Decoder(nn.Module):
    def __init__(
        self,
        model_name="/home/caoqian/playground/GPT2-Chinese-master/pretrain/gpt2-chinese-cluecorpussmall",
    ...
    def forward(self, agg_output, input_ids, attention_mask=None):
        '''
        Params:
            agg_output: [batch_size, agg_output_dim]
            input_ids: [batch_size, _max_sent_length*2]
        '''
        # import pdb; pdb.set_trace()
        labels = input_ids
        attention_mask = attention_mask
        input_ids_np = input_ids.cpu().tolist() # [batch_size, _max_sent_length*2]
        # agg_output = agg_output.cpu().tolist() # [batch_size, agg_output_dim]
        ...
        # import pdb; pdb.set_trace()
        # each token in input_ids_wenlan multiply the corresponding token in agg_output
        # [batch_size, _max_sent_length*2, 2048] * [batch_size, _max_sent_length*2, agg_output_dim] => [batch_size, _max_sent_length*2, agg_output_dim]
        agg_output = agg_output.unsqueeze(1).repeat(1,input_ids.size(1),1)
        input_ids_ = input_ids_wenlan * agg_output # [batch_size, _max_sent_length*2, 2048]

        # test if use img_emb as prompt
        # input_ids_ = torch.cat([torch.cat([input_ids_[:,0].unsqueeze(1), img_embs.unsqueeze(1)], dim=1), input_ids_[:,1:]], dim=1)

        # print(img_embs.shape, labels.shape)
        out1 = self.projector_layer1(input_ids_)
        out1 = self.tanh(out1)
        gpt_input_embs = self.projector_layer2(out1)
        res = self.gpt2(
            inputs_embeds=gpt_input_embs.squeeze(1),
            # attention_mask=attention_mask,
            labels=labels,
            return_dict=True,
        ) # ['loss', 'logits', 'past_key_values']
        # print("loss: ", res['loss'])
        return res['loss'], res['logits'] # [batch_size, seq_len, vocab_size]
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        mm_attention_output = self.mm_atten_layer(topic_output, \
            img_self_attention_output.transpose(0,1), text_self_attention_output.transpose(0,1))
        ...
        for i in range(batch_size):
            for j in range(seq_len):
                loss[i][j], outputs[i][j] = self.decoder(output_agg[i,j,:].unsqueeze(0), decoder_input[i,j,:].unsqueeze(0))
        # outputs = outputs.transpose(0, 1)
        
        return loss, outputs # [batch_size, seq_len, _sent_length*2, vocab_size]
