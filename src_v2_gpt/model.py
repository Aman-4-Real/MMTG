...
LastEditors: Aman
LastEditTime: 2021-12-24 00:58:31
'''
...
class GPT2_Decoder(nn.Module):
    ...
    def forward(self, agg_output, input_ids, attention_mask=None):
        ...
        agg_output = agg_output.cpu().tolist()
        for i in range(len(input_ids_np)):
            # import pdb; pdb.set_trace()
            embs = [list(map(lambda x:x[0]+x[1], zip(self.token_id2emb[id], agg_output[i]))) for id in input_ids_np[i]]
            embs = np.asarray(embs, dtype=np.float32)
            input_ids_np[i] = embs
        input_ids_np = np.asarray(input_ids_np, dtype=np.float32)
        input_ids_ = torch.from_numpy(input_ids_np).to(input_ids.device)
        # import pdb; pdb.set_trace()
        # img_embs = img_embs[0][0].cpu().tolist()
        # img_embs = np.asarray(img_embs, dtype=np.float32)
        # img_embs = torch.from_numpy(img_embs).to(input_ids.device).unsqueeze(0).unsqueeze(0)
        # out1 = self.projector_layer1(img_embs)
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        outputs = torch.zeros(seq_len, batch_size, decoder_input.shape[2], self.vocab_size).to(device)
        loss = torch.zeros(seq_len).to(device)
        for i in range(seq_len):
            loss[i], outputs[i] = self.decoder(output_agg[:,i,:], decoder_input[:,i,:]) # , batch['img_embs']
