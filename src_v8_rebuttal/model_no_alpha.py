...
LastEditors: Aman
LastEditTime: 2022-06-09 12:11:02
'''
...
class MultiModalEncoder(nn.Module):
    def __init__(self, model_cfgs):
        ...
        if model_cfgs['image']['type'] == 'RNN':
            ...
            self.rnns_image = nn.GRU(self.image_input_dim, self.image_hidden_dim, \
                                    num_layers=self.image_num_layers, dropout=self.dropout_rate)
        elif model_cfgs['image']['type'] == 'TRM':
            self.linear_image = nn.Linear(self.image_input_dim, self.image_hidden_dim)
            encoder_layer = nn.TransformerEncoderLayer(d_model=self.image_hidden_dim, nhead=4)
            self.rnns_image = nn.TransformerEncoder(encoder_layer, num_layers=1)
        ...
        if model_cfgs['text']['type'] == 'RNN':
            ...
            self.rnns_text = nn.GRU(self.text_input_dim, self.text_hidden_dim, \
                                    num_layers=self.text_num_layers, dropout=self.dropout_rate)
        elif model_cfgs['text']['type'] == 'TRM':
            self.linear_text = nn.Linear(self.text_input_dim, self.text_hidden_dim)
            encoder_layer = nn.TransformerEncoderLayer(d_model=self.text_hidden_dim, nhead=4)
            self.rnns_text = nn.TransformerEncoder(encoder_layer, num_layers=1)
    ...
    def forward(self, encoder_batch):
        ...
        '''
        if self.model_cfgs['text']['type'] in ['RNN', 'LSTM', 'GRU']:
        ...
        if self.model_cfgs['text']['type'] in ['RNN', 'LSTM', 'GRU']:
            ...
            output_image = self.linear_image(x_image)
            output_image = self.rnns_image(output_image)
            output_text = self.linear_text(x_text)
            output_text = self.rnns_text(output_text)
    ...
    def init_weights(self):
        ...
        if self.model_cfgs['text']['type'] in ['RNN', 'LSTM', 'GRU']:
            ...
            init.xavier_normal_(self.linear_image.weight)
            init.xavier_normal_(self.linear_text.weight)
...
class InnerModalAttentionLayer(nn.Module):
    def __init__(self, model_cfgs):
        ...
        self.normal_dists = torch.zeros((5, 5), dtype=torch.float32)
        for i in range(5):
            ...
            self.normal_dists[i] = normal_values
    ...
    def forward(self, input):
        ...
        self.normal_dists = self.normal_dists.to(attention_probs.device).unsqueeze(0).unsqueeze(0).repeat(attention_probs.size(0), attention_probs.size(1), 1, 1)
        context_layer = torch.matmul(self.normal_dists, value_layer)
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        # print("text att: ")
        text_inner_attention_output = self.text_inner_atten_layer(text_output.transpose(0, 1))
