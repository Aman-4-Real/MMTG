...
LastEditors: Aman
LastEditTime: 2022-01-21 15:29:36
'''
...
class MultiModalAttentionLayer(nn.Module):
    ...
    def forward(self, topic_output, image_output):
        ...
        for i in range(self.seq_len):
            atten_input = torch.cat([topic_output, image_output[i,:,:].unsqueeze(0)], dim=0) # [2, batch_size, hidden_dim]
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        mm_attention_output = self.mm_atten_layer(topic_output, \
                                img_self_attention_output.transpose(0,1))
