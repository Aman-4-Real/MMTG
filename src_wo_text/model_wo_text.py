...
LastEditors: Aman
LastEditTime: 2022-04-01 17:12:25
'''
...
class MultiModalAttentionLayer(nn.Module):
    ...
    def forward(self, topic_output, image_output):
        ...
        for i in range(self.seq_len):
            ...
            output = torch.bmm(
                atten, torch.cat([topic_output.transpose(0, 1), \
                image_output[i,:,:].unsqueeze(0).transpose(0, 1)], dim=1)) # [batch_size, 1, 3] * [batch_size, 3, hidden_dim] => [batch_size, 1, hidden_dim]
            # print(atten)
