'''
Author: Aman
Date: 2022-04-04 22:18:18
Contact: cq335955781@gmail.com
LastEditors: Aman
LastEditTime: 2022-04-04 22:48:16
'''
...
class MultiModalEncoder(nn.Module):
    def __init__(self, model_cfgs):
        ...
        assert self.topic_hidden_dim == self.image_hidden_dim, \
            "The hidden dim of topic and image must be equal."
        # for topic mlp
    ...
    def forward(self, encoder_batch):
        ...
        return output_topic, output_image
...
class MultiModalAttentionLayer(nn.Module):
    ...
    def forward(self, topic_output, image_output):
        '''
        Args:
        ...
        for i in range(self.seq_len):
            ...
            atten = nn.Softmax(dim=-1)(torch.cat([topic_att, image_att], dim=1).permute(0,2,1)) # [batch_size, attention_dim, 3] => [batch_size, 1, 3]
            output = torch.bmm(
                atten, torch.cat([topic_output.transpose(0, 1), image_output[i,:,:].unsqueeze(0).transpose(0, 1)], dim=1)) # [batch_size, 1, 3] * [batch_size, 3, hidden_dim] => [batch_size, 1, hidden_dim]
            # print(atten)
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        # ===== Multi-modal Encoder =====
        topic_output, image_output = self.encoder(encoder_batch)
        ...
        mm_attention_output = self.mm_atten_layer(topic_output, img_inner_attention_output.transpose(0,1))
        ...
        return loss, img_kl_loss.mean(), outputs # [batch_size, seq_len+_max_seq_length, vocab_size]
