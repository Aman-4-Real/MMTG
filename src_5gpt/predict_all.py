...
def main():
    ...
    # =====> generate samples <=====
    while 1:
        f1 = open("res/samples_final_5gpt_ep3.txt", "w", encoding="utf-8")
            ...
            for _ in range(n_samples):
                ...
                for i, line in enumerate(preds):
                    ...
                    if len(all_idx_of_eos) >= 2 and '[SEP]' not in line[:all_idx_of_eos[-1]]:
                        ...
                        line = line + ['[SEP]']
                    if i >= 1:
                        line = ''.join(line).replace('[#START#]', '')
                    whole_preds += line
                n_preds += [("".join(whole_preds).replace('[#EOS#][SEP]','[SEP]')).replace('[#EOS#]', '，').replace('[#START#]', '').replace('[SEP]', '')]
