...
LastEditors: Aman
LastEditTime: 2022-01-26 00:56:14
'''
...
class GPT2_Decoder(nn.Module):
    ...
    def forward(self, agg_output, input_ids, attention_mask=None):
        ...
        out1 = self.tanh(out1)
        gpt_input_embs = self.projector_layer2(out1).squeeze(1)
        res = self.gpt2(
...
class EXPTeller(nn.Module):
    ...
    def forward(self, batch):
        ...
        for i in range(seq_len):
            loss[i], outputs[i] = self.decoder(concat_output[:,i,:], decoder_input[:,i,:]) # , batch['img_embs']
