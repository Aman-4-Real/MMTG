...
LastEditors: Aman
LastEditTime: 2022-03-16 01:44:48
'''
...
parser.add_argument("--save_path", default="./models/lr1e-6_bs96_kl02", type=str, help="Save directory")
# parser.add_argument("--save_interval", default=1, type=int, help="Save interval")
parser.add_argument("--log_path", default="./logs/lr1e-6_bs96_kl02.log", type=str, help="Log directory")
parser.add_argument("--tensorboard_log_dir", default="./logs/lr1e-6_bs96_kl02", type=str, help="Tensorboard log directory")
...
class MyLoss(torch.nn.Module):
    ...
    def forward(self, outputs, targets, ratings, stage):
        ...
        # shift_logits = [outputs[:, i*45+1:(i+1)*45, :] for i in range(self._seq_len)]
        # shift_labels = [targets[:, i*44+1:(i+1)*44+1] for i in range(self._seq_len)]
        shift_logits = outputs[:, 1:45, :]
        for i in range(1, self._seq_len):
            shift_logits = torch.cat([shift_logits, outputs[:, i*45+1:(i+1)*45, :]], dim=1)
